...
# Version: 5.3 - Speed Optimized Team Context + Regularization Edition
...
=========================
project_folder/
├── forward_points_model_v5.3.py (this script)
...
- DATA_OUTPUT INTEGRATION: Designed to work with merged data from NHL data merger script

SPEED OPTIMIZATIONS IMPLEMENTED (v5.3):
=======================================
...
- Searches for CSV file in data_output folder first
...
def find_csv_file() -> str:
    ...
    print("\nExpected folder structure:")
    print(f"  {current_dir}/")
    print(f"  ├── forward_points_model_v5.3.py (this script)")
    print(f"  ├── data_output/")
...
class SpeedOptimizedHockeyPointsPredictorWithTeamContext:
    ...
    Designed to work with merged player-team data from the NHL data merger script located 
    in the data_output folder.
    """
    ...
    def load_data(self, csv_path: str) -> pd.DataFrame:
        """
        Load hockey player statistics from merged player-team CSV file and remove defensemen.
        
        This function performs initial data cleaning and preparation:
        - Loads merged player-team data from data_output folder
        - Removes defensemen to focus on forwards only
        - Parses seasons and extracts year information
        - Shows data availability by year and position
        
        Args:
            csv_path: Path to the merged player-team CSV file (typically data_output/skater_team_data.csv)
    ...
    def create_lag_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Works with merged player-team data from the data_output folder.
        
        Args:
            df: DataFrame with merged player-team statistics by year
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Args:
            df: DataFrame with lag features from merged player-team data
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        individual and team context from the merged player-team data.
        
        Args:
            df: DataFrame with engineered features from merged player-team data
    ...
    def predict_season(self, df_full: pd.DataFrame, target_year: int) -> pd.DataFrame:
        ...
        Args:
            df_full: Complete dataset including target year (merged player-team data)
...
def main():
    ...
    Uses dynamic path detection with primary expectation of data in data_output folder.
    """
    
    print("Speed Optimized Hockey Points Predictor v5.3 - Team Context + Robust Regularization")
    print("=" * 90)
    print(f"Run started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"SPEED OPTIMIZATIONS: ~70% faster execution with team context + maintained prediction quality")
    print(f"PRIMARY DATA SOURCE: data_output/skater_team_data.csv (merged player-team data)")
    ...
    try:
        ...
        print(f"\nERROR: {e}")
        print("Please ensure your merged player-team CSV file is accessible in the data_output folder and try again.")
        return
    except Exception as e:
    ...
    output_file = os.path.join(results_dir, f'speed_optimized_team_context_forward_points_prediction_v5.3_{timestamp}.txt')
    ...
    try:
        ...
        print(f"SPEED OPTIMIZATIONS: ~70% faster execution with team context + maintained prediction quality")
        print(f"PRIMARY DATA SOURCE: Merged player-team data from NHL data merger script")
        ...
        for config in configs:
            ...
            try:
                ...
                print("Loading merged player-team data...")
        ...
        if best_results_all:
            ...
            plot_filename = os.path.join(plots_dir, f'speed_optimized_team_context_forward_model_v5.3_all_data_{timestamp}.png')
        ...
        if best_results_pred and best_predictions is not None:
            ...
            plot_filename = os.path.join(plots_dir, f'speed_optimized_team_context_forward_model_v5.3_prediction_{timestamp}.png')
        ...
        if best_results_all:
            print(f"All data plot saved to: {os.path.join(plots_dir, f'speed_optimized_team_context_forward_model_v5.3_all_data_{timestamp}.png')}")
        if best_results_pred:
            print(f"Prediction plot saved to: {os.path.join(plots_dir, f'speed_optimized_team_context_forward_model_v5.3_prediction_{timestamp}.png')}")
        ...
        print("• Data Output Integration: Designed for merged player-team data workflow")
