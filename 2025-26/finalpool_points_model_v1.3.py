"""
NHL POOL RANKING SYSTEM - UNIFIED MODEL v1.3
=============================================
Date: September 16, 2025
Author: Updated from v1.2 with per-game rate predictions
...
CHANGES IN v1.3:
================
- Forwards/Defensemen: Predicts per-game rates, then multiplies by career average GP
- No normalization - uses actual expected games played for predictions
- Goalies: Continue using 3-year average games played
- More accurate projections based on per-game performance rates
...
3. POSITION-SPECIFIC MODELS:
   ...
   - Normalization: Predictions scaled to career average GP
   ...
   - Normalization: Both predictions scaled to career average GP
   ...
   - Games Played: 3-year average for predictions
   - Pool Points: Wins + 3×Shutouts + GAA bonus + Save% bonus

4. PREDICTION NORMALIZATION:
   - Forwards/Defensemen: Predicts per-game rates, multiplies by career average GP
   - Goalies: Predicts per-game rates, multiplies by 3-year average games played
...
C. Predict 2025-26 season with team reassignments and career GP normalization

OUTPUT STRUCTURE:
================
- All plots saved to: nhl_plots/
- All results saved to: nhl_results/
- File naming: finalpool_[description]_v1.3_[timestamp].[ext]
"""
...
class UnifiedNHLPoolPredictor:
    ...
    v1.3: Per-game rate predictions multiplied by career average GP for skaters.
    """
    ...
    def __init__(self, lag_years: int = 2, output_dir: str = None):
        ...
        self.forward_career_gp = {}
        self.defense_career_gp = {}
    ...
    def load_and_prepare_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        self._calculate_forward_career_gp(forwards_df)
        self._calculate_defense_career_gp(defensemen_df)
    ...
    def _calculate_defense_career_gp(self, df: pd.DataFrame):
        ...
        for player_id in df['player_id'].unique():
            ...
            if len(valid_gp) > 0:
                avg_gp = np.mean(valid_gp)
                # Cap at 82 games but keep realistic averages
                self.defense_career_gp[player_id] = min(avg_gp, 82)
            else:
                self.defense_career_gp[player_id] = 72  # Default for unknown defensemen
        
        print(f"Calculated career average GP for {len(self.defense_career_gp)} defensemen")
        print(f"Average career GP across all defensemen: {np.mean(list(self.defense_career_gp.values())):.1f}")
    ...
    def predict_season(self, df_full: pd.DataFrame, target_year: int, 
        ...
        v1.3: Predicts per-game rates and multiplies by expected games played.
        ...
        if position == 'forward':
            # Get career average GP for each player
            for idx in results_df.index:
                player_id = target_data.loc[idx, 'player_id']
                career_gp = self.forward_career_gp.get(player_id, 70)
                results_df.loc[idx, 'projected_games'] = career_gp
            ...
            if self.models['forward']['points'] is not None and len(X) > 0:
                ...
                if selector_key in self.selected_features:
                    ...
                    # Multiply by expected games played to get season totals
                    for idx in X_valid.index:
                        player_id = target_data.loc[idx, 'player_id']
                        career_gp = self.forward_career_gp.get(player_id, 70)
                        season_total = per_game_predictions[X_valid.index.get_loc(idx)] * career_gp
            ...
            for idx in results_df.index:
                player_id = target_data.loc[idx, 'player_id']
                career_gp = self.defense_career_gp.get(player_id, 72)
                results_df.loc[idx, 'projected_games'] = career_gp
            ...
            if self.models['defense']['points'] is not None and len(X_points) > 0:
                ...
                if selector_key in self.selected_features:
                    ...
                    # Multiply by expected games played
                    for idx in X_valid.index:
                        player_id = target_data.loc[idx, 'player_id']
                        career_gp = self.defense_career_gp.get(player_id, 72)
                        season_total = per_game_points[X_valid.index.get_loc(idx)] * career_gp
                        results_df.loc[idx, 'predicted_points'] = season_total
            ...
            if self.models['defense']['plus_minus'] is not None and len(X_pm) > 0:
                ...
                if selector_key in self.selected_features:
                    ...
                    for idx in X_valid.index:
                        player_id = target_data.loc[idx, 'player_id']
                        career_gp = self.defense_career_gp.get(player_id, 72)
                        season_total = per_game_pm[X_valid.index.get_loc(idx)] * career_gp
            ...
            # Get expected games played from 3-year average
    ...
    def generate_comprehensive_rankings(self, forwards_pred: pd.DataFrame, 
        ...
        # Save to CSV
        csv_path = f'{self.results_dir}/finalpool_{scenario_name}_v1.3_{self.timestamp}.csv'
        all_players.to_csv(csv_path, index=False)
        print(f"\nSaved rankings to: {csv_path}")
        
        # Save summary text file
        txt_path = f'{self.results_dir}/finalpool_{scenario_name}_summary_v1.3_{self.timestamp}.txt'
        with open(txt_path, 'w') as f:
            f.write(f"NHL POOL RANKINGS - {scenario_name.upper()}\n")
            f.write("="*80 + "\n")
            f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Model Version: 1.3 (Per-game rate predictions)\n\n")
            
            f.write("TOP 50 OVERALL RANKINGS\n")
            f.write("-"*80 + "\n")
            f.write(f"{'Rank':<6} {'Player':<30} {'Team':<5} {'Pos':<4} {'Pool Pts':<10} {'Proj GP':<8}\n")
            f.write("-"*80 + "\n")
            
            for _, row in all_players.head(50).iterrows():
                proj_gp = row['projected_games'] if 'projected_games' in row else 'N/A'
    ...
    def create_visualization(self, rankings: pd.DataFrame, results: Dict[str, Any], 
        ...
        ax1.set_xlabel('Pool Points (Career GP-based)')
        ...
        ax2.set_xlabel('Pool Points (Career GP-based)')
        ...
        ax3.set_xlabel('Pool Points (3-yr avg GP)')
        ...
        ax4.set_title('Top 20 Overall Players (Per-game rates × Career GP)')
        ...
        plt.suptitle(f'NHL Pool Rankings - {scenario_name} (v1.3)', fontsize=16, y=0.98)
        
        # Save plot
        plot_path = f'{self.plots_dir}/finalpool_{scenario_name}_v1.3_{self.timestamp}.png'
...
def main():
    ...
    print("\n" + "="*80)
    print("NHL POOL RANKING SYSTEM v1.3")
    print("PER-GAME RATE PREDICTIONS WITH CAREER AVERAGE GAMES PLAYED")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("SCENARIO B: PREDICTING 2024-25 SEASON (PER-GAME RATES)")
    print("="*80)
    ...
    # Predict 2024-25 with per-game rates
    ...
    rankings_2024 = predictor_2024.generate_comprehensive_rankings(
        forwards_2024, defense_2024, goalies_2024, "2024_25_per_game_rates"
    )
    
    # Create visualization
    predictor_2024.create_visualization(rankings_2024, results_2024, "2024_25_per_game_rates")
    
    # Display top 50
    print("\n" + "="*80)
    print("TOP 50 PREDICTED POOL RANKINGS FOR 2024-25 (PER-GAME RATES)")
    print("="*80)
    print(f"{'Rank':<6} {'Player':<30} {'Team':<5} {'Position':<12} {'Pool Points':<12} {'Proj GP':<8}")
    print("-"*85)
    
    for _, row in rankings_2024.head(50).iterrows():
        proj_gp = row['projected_games'] if 'projected_games' in row else 'N/A'
        print(f"{row['overall_rank']:<6} {row['player_name'][:29]:<30} {row['team_abbrev']:<5} "
              f"{row['position_group']:<12} {row['pool_points']:<12.1f} {proj_gp:<8.0f}")
    
    # SCENARIO C: Predict 2025-26 with team changes
    print("\n" + "="*80)
    print("SCENARIO C: PREDICTING 2025-26 WITH TEAM CHANGES (PER-GAME RATES)")
    print("="*80)
    ...
    rankings_2025 = predictor_2024.generate_comprehensive_rankings(
        forwards_2025, defense_2025, goalies_2025, "2025_26_per_game_with_trades"
    )
    
    # Create visualization
    predictor_2024.create_visualization(rankings_2025, results_2024, "2025_26_per_game_with_trades")
    ...
    print(f"  Forwards: Per-game rates × Career avg GP ({np.mean(list(predictor_2024.forward_career_gp.values())):.1f} games avg)")
    print(f"  Defensemen: Per-game rates × Career avg GP ({np.mean(list(predictor_2024.defense_career_gp.values())):.1f} games avg)")
    print(f"  Goalies: Per-game rates × 3-year average games played")
