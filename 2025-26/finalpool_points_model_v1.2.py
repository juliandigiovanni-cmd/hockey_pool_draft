"""
NHL POOL RANKING SYSTEM - UNIFIED MODEL v1.2
=============================================
Date: September 16, 2025
Author: Updated from v1.1 with career average GP normalization
...
CHANGES IN v1.2:
================
- Forwards/Defensemen: Predictions normalized to career average games played (instead of fixed 82)
- Goalies: Continue using 3-year average games played
- More realistic projections based on individual player durability
...
4. PREDICTION NORMALIZATION:
   - Forwards/Defensemen: All predictions normalized to career average GP
   - Goalies: Predictions based on 3-year average games played
...
- File naming: finalpool_[description]_v1.2_[timestamp].[ext]
"""
...
class UnifiedNHLPoolPredictor:
    ...
    v1.2: Career average GP normalization for skaters, 3-year average GP for goalies.
    """
    ...
    def predict_season(self, df_full: pd.DataFrame, target_year: int, 
                      ...
                      normalize_to_career_gp: bool = True) -> pd.DataFrame:
        """
        ...
        Args:
            ...
            normalize_to_career_gp: Whether to normalize predictions to career average GP
        ...
        if position == 'forward':
            ...
            if self.models['forward']['points'] is not None and len(X) > 0:
                ...
                if selector_key in self.selected_features:
                    ...
                    # Normalize to career GP if requested
                    if normalize_to_career_gp:
                        for idx in X_valid.index:
                            player_id = target_data.loc[idx, 'player_id']
                            career_gp = self.forward_career_gp.get(player_id, 70)
                            hist_gp_cols = [col for col in X_valid.columns if 'games_played' in col and 'lag' in col]
                            if hist_gp_cols:
                                hist_gp = X_valid.loc[idx, hist_gp_cols].mean()
                                if hist_gp > 0:
                                    scaling_factor = career_gp / hist_gp
                                    predictions[X_valid.index.get_loc(idx)] *= scaling_factor
                    ...
                    if normalize_to_career_gp:
                        for idx in X_valid.index:
                            ...
                            if hist_gp_cols:
                                ...
                                if hist_gp > 0:
                                    scaling_factor = career_gp / hist_gp
                                    points_pred[X_valid.index.get_loc(idx)] *= scaling_factor
                    ...
                    if normalize_to_career_gp:
                        for idx in X_valid.index:
                            ...
                            if hist_gp_cols:
                                ...
                                if hist_gp > 0:
                                    scaling_factor = career_gp / hist_gp
                                    pm_pred[X_valid.index.get_loc(idx)] *= scaling_factor
            ...
            if self.models['goalie']['wins'] is not None and len(X_wins) > 0:
                ...
                if selector_key in self.selected_features:
                    ...
                    # Scale by projected games vs historical games
                    for idx in X_valid.index:
                        player_id = target_data.loc[idx, 'player_id']
                        ...
                        if hist_gp_cols:
                            hist_gp = X_valid.loc[idx, hist_gp_cols].mean()
                            if hist_gp > 0:
                                scaling_factor = projected_gp / hist_gp
                                predictions[X_valid.index.get_loc(idx)] *= scaling_factor
    ...
    def generate_comprehensive_rankings(self, forwards_pred: pd.DataFrame, 
        ...
        # Save to CSV
        csv_path = f'{self.results_dir}/finalpool_{scenario_name}_v1.2_{self.timestamp}.csv'
        all_players.to_csv(csv_path, index=False)
        print(f"\nSaved rankings to: {csv_path}")
        
        # Save summary text file
        txt_path = f'{self.results_dir}/finalpool_{scenario_name}_summary_v1.2_{self.timestamp}.txt'
        with open(txt_path, 'w') as f:
            f.write(f"NHL POOL RANKINGS - {scenario_name.upper()}\n")
            f.write("="*80 + "\n")
            f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Model Version: 1.2 (Career GP normalized)\n\n")
    ...
    def create_visualization(self, rankings: pd.DataFrame, results: Dict[str, Any], 
        ...
        ax1.set_xlabel('Pool Points (Career GP normalized)')
        ...
        ax2.set_xlabel('Pool Points (Career GP normalized)')
        ...
        ax4.set_title('Top 20 Overall Players (Career GP normalized)')
        ...
        plt.suptitle(f'NHL Pool Rankings - {scenario_name} (v1.2)', fontsize=16, y=0.98)
        
        # Save plot
        plot_path = f'{self.plots_dir}/finalpool_{scenario_name}_v1.2_{self.timestamp}.png'
...
def main():
    ...
    print("\n" + "="*80)
    print("NHL POOL RANKING SYSTEM v1.2")
    print("CAREER AVERAGE GP NORMALIZED PREDICTIONS")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("SCENARIO B: PREDICTING 2024-25 SEASON (CAREER GP NORMALIZED)")
    print("="*80)
    ...
    # Predict 2024-25 with career GP normalization
    forwards_2024 = predictor_2024.predict_season(forwards_full, 2024, 'forward', normalize_to_career_gp=True)
    defense_2024 = predictor_2024.predict_season(defensemen_full, 2024, 'defense', normalize_to_career_gp=True)
    ...
    rankings_2024 = predictor_2024.generate_comprehensive_rankings(
        forwards_2024, defense_2024, goalies_2024, "2024_25_career_gp_normalized"
    )
    
    # Create visualization
    predictor_2024.create_visualization(rankings_2024, results_2024, "2024_25_career_gp_normalized")
    
    # Display top 50
    print("\n" + "="*80)
    print("TOP 50 PREDICTED POOL RANKINGS FOR 2024-25 (CAREER GP NORMALIZED)")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("SCENARIO C: PREDICTING 2025-26 WITH TEAM CHANGES (CAREER GP NORMALIZED)")
    print("="*80)
    ...
    print("All forwards and defensemen normalized to career average games played")
    print("Goalies predicted based on 3-year average games played")
    
    # Predict with team changes and career GP normalization
    forwards_2025 = predictor_2024.predict_season(forwards_full, 2024, 'forward', team_changes, normalize_to_career_gp=True)
    defense_2025 = predictor_2024.predict_season(defensemen_full, 2024, 'defense', team_changes, normalize_to_career_gp=True)
    ...
    print(f"  Forwards: Career average GP ({np.mean(list(predictor_2024.forward_career_gp.values())):.1f} games avg)")
    print(f"  Defensemen: Career average GP ({np.mean(list(predictor_2024.defense_career_gp.values())):.1f} games avg)")
    print(f"  Goalies: 3-year average games played")
