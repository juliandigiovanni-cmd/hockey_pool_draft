"""
NHL POOL RANKING SYSTEM - UNIFIED MODEL v1.1
=============================================
Date: September 13, 2025
Author: Updated from v1.0 with 82-game normalization
...
3. POSITION-SPECIFIC MODELS:
   ...
   - Normalization: Predictions scaled to 82-game season
   ...
   - Normalization: Both predictions scaled to 82-game season
...
4. PREDICTION NORMALIZATION:
   - Forwards/Defensemen: All predictions normalized to 82-game schedule
...
C. Predict 2025-26 season with team reassignments and 82-game normalization

OUTPUT STRUCTURE:
================
- All plots saved to: nhl_plots/
- All results saved to: nhl_results/
- File naming: finalpool_[description]_v1.1_[timestamp].[ext]
"""
...
class UnifiedNHLPoolPredictor:
    ...
    v1.1: Added 82-game normalization for skaters and 3-year average GP for goalies.
    """
    ...
    def __init__(self, lag_years: int = 2, output_dir: str = None):
        ...
        self.full_season_games = 82  # NHL regular season games
        ...
        # Store historical games played for goalies
    ...
    def load_and_prepare_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        # Calculate 3-year average games played for goalies
    ...
    def predict_season(self, df_full: pd.DataFrame, target_year: int, 
                      ...
                      normalize_to_82_games: bool = True) -> pd.DataFrame:
        """
        ...
        Args:
            ...
            normalize_to_82_games: Whether to normalize predictions to 82-game season
        ...
        if position == 'forward':
            ...
            if self.models['forward']['points'] is not None and len(X) > 0:
                ...
                if selector_key in self.selected_features:
                    ...
                    # Normalize to 82 games if requested
                    if normalize_to_82_games:
                        # Get historical games played average
                        for idx in X_valid.index:
                            ...
                            if hist_gp_cols:
                                ...
                                if hist_gp > 0:
                                    scaling_factor = self.full_season_games / hist_gp
                    ...
                    results_df.loc[X_valid.index, 'projected_games'] = self.full_season_games
            ...
            results_df['projected_games'] = self.full_season_games
    ...
    def generate_comprehensive_rankings(self, forwards_pred: pd.DataFrame, 
        ...
        # Save to CSV
        csv_path = f'{self.results_dir}/finalpool_{scenario_name}_v1.1_{self.timestamp}.csv'
        all_players.to_csv(csv_path, index=False)
        print(f"\nSaved rankings to: {csv_path}")
        
        # Save summary text file
        txt_path = f'{self.results_dir}/finalpool_{scenario_name}_summary_v1.1_{self.timestamp}.txt'
        with open(txt_path, 'w') as f:
            f.write(f"NHL POOL RANKINGS - {scenario_name.upper()}\n")
            f.write("="*80 + "\n")
            f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Model Version: 1.1 (82-game normalized)\n\n")
    ...
    def create_visualization(self, rankings: pd.DataFrame, results: Dict[str, Any], 
        ...
        ax1.set_xlabel('Pool Points (82-game normalized)')
        ...
        ax2.set_xlabel('Pool Points (82-game normalized)')
        ...
        ax4.set_title('Top 20 Overall Players (82-game normalized)')
        ...
        plt.suptitle(f'NHL Pool Rankings - {scenario_name} (v1.1)', fontsize=16, y=0.98)
        
        # Save plot
        plot_path = f'{self.plots_dir}/finalpool_{scenario_name}_v1.1_{self.timestamp}.png'
...
def main():
    ...
    print("\n" + "="*80)
    print("NHL POOL RANKING SYSTEM v1.1")
    print("3-YEAR LAG FEATURES WITH 82-GAME NORMALIZED PREDICTIONS")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("SCENARIO B: PREDICTING 2024-25 SEASON (82-GAME NORMALIZED)")
    print("="*80)
    ...
    # Predict 2024-25 with 82-game normalization
    forwards_2024 = predictor_2024.predict_season(forwards_full, 2024, 'forward', normalize_to_82_games=True)
    defense_2024 = predictor_2024.predict_season(defensemen_full, 2024, 'defense', normalize_to_82_games=True)
    ...
    rankings_2024 = predictor_2024.generate_comprehensive_rankings(
        forwards_2024, defense_2024, goalies_2024, "2024_25_82game_normalized"
    )
    
    # Create visualization
    predictor_2024.create_visualization(rankings_2024, results_2024, "2024_25_82game_normalized")
    
    # Display top 50
    print("\n" + "="*80)
    print("TOP 50 PREDICTED POOL RANKINGS FOR 2024-25 (82-GAME NORMALIZED)")
    print("="*80)
    print(f"{'Rank':<6} {'Player':<30} {'Team':<5} {'Position':<12} {'Pool Points':<12} {'Proj GP':<8}")
    print("-"*85)
    
    for _, row in rankings_2024.head(50).iterrows():
        proj_gp = row['projected_games'] if 'projected_games' in row else 82
    ...
    print("\n" + "="*80)
    print("SCENARIO C: PREDICTING 2025-26 WITH TEAM CHANGES (82-GAME NORMALIZED)")
    print("="*80)
    ...
    print("All forwards and defensemen normalized to 82-game season")
    print("Goalies predicted based on 3-year average games played")
    
    # Predict with team changes and 82-game normalization
    forwards_2025 = predictor_2024.predict_season(forwards_full, 2024, 'forward', team_changes, normalize_to_82_games=True)
    defense_2025 = predictor_2024.predict_season(defensemen_full, 2024, 'defense', team_changes, normalize_to_82_games=True)
    ...
    rankings_2025 = predictor_2024.generate_comprehensive_rankings(
        forwards_2025, defense_2025, goalies_2025, "2025_26_82game_with_trades"
    )
    
    # Create visualization
    predictor_2024.create_visualization(rankings_2025, results_2024, "2025_26_82game_with_trades")
    ...
    print(f"  Forwards/Defensemen: 82-game season")
