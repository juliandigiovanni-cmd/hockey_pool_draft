...
common/               shared library, reused every season
  ...
  draft/
    ...
    strategy_sim.py           Monte Carlo position-order policy comparison
...
\begin{description}[leftmargin=3cm, style=nextline]
  ...
  \item[Interactive draft-day tool (player-level, live)]
        The \emph{position-order} strategy piece is now built (Section~\ref{sec:draftstrategy},
        \texttt{draft\_strategy.py}). What remains is the live, player-level assistant: show
        remaining available players ranked by pool points during the actual draft, allow
        filtering by position, mark picked players as they come off the board, and recompute
        recommendations in real time. Deliberately deferred and scoped separately — the
        position-order tool above is intentionally player-identity-free.
