...
Each player's projected games played is the 3-year rolling average of their actual
\texttt{games\_played\_player}, capped at \texttt{cfg.games\_per\_season}:

\begin{equation}
  \text{GP}_i^{\text{proj}} = \min\!\left(\frac{1}{3}\sum_{k=1}^{3} \text{GP}_{i,t-k},\;
  \texttt{games\_per\_season}\right)
\end{equation}

For players with no recent history: forwards and defense default to 70 games; goalies default
to 20 games. This reflects injury risk and roster uncertainty rather than assuming a full
84-game season for everyone.
